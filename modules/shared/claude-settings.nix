...
let
  ...
  settingsGlmTemplate = pkgs.writeText "settings-glm.json.tpl" (builtins.toJSON settingsGlmJson);
...
in
{
  ...
  home.activation.claudeSettingsGlm = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    mkdir -p "$HOME/.claude"
    cp ${settingsGlmTemplate} "$HOME/.claude/settings-glm.json.tpl"
    if ${op}/bin/op inject -i ${settingsGlmTemplate} -o "$HOME/.claude/settings-glm.json"; then
